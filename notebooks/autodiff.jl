...
# ╔═╡ cfd10ad6-1052-4f13-9b32-e52a55330445
md"""
## Automatic
"""
...
# ╔═╡ 0fe889c1-0d72-49ee-83a3-7a0e279f063e
md"""
...
# ╔═╡ 2b54e622-58e6-4b2b-837a-a4bfc4d07955
A, b = rand(2, 3), rand(2)

# ╔═╡ 305f2cc7-782d-4963-9142-b3aeb8b5b904
md"""
Here comes the chain rule.
"""

# ╔═╡ 39b865fd-72e6-4fbe-9e89-bb9f2f08882b
function ∂(f::ComposedFunction)
    g, h = f.outer, f.inner
    return x -> ∂(g)(h(x)) ∘ ∂(h)(x)
end

# ╔═╡ 45bc2f6e-524d-480f-b17a-12ddfb39a0cd
function ∂ᵀ(f::ComposedFunction)
    g, h = f.outer, f.inner
    return x -> ∂ᵀ(h)(x) ∘ ∂ᵀ(g)(h(x))
end

# ╔═╡ cc179bf7-b56a-4f9e-9a33-1414643ca715
md"""
Does it work?
"""

# ╔═╡ 663f8f45-16ad-4e26-ab5f-0c4af4b88661
md"""
# Sparsity handling
"""

# ╔═╡ 0e746ec0-f6ab-4e81-93ce-f13afc271bb7
md"""
## Principle
"""
...
let
	...
	@time heron(x, L_slider)
end

# ╔═╡ 15b4c88c-2707-4ede-b12f-174be0d179a0
let
	x = Symbolics.variable(:x)
	@time Symbolics.derivative(heron(x, L_slider), x)
end
...
let
	...
	hs = 10.0 .^ (-15:0.2:0)
	finitediff_errs = map(
		h -> abs(finitediff_der(heron, x0; h) - inv(2 * sqrt(x0))), hs
	)
	finitediff_central_errs = map(
		h -> abs(finitediff_central_der(heron, x0; h) - inv(2 * sqrt(x0))), hs
	)
	finitediff_complex_errs = map(
		h -> abs(finitediff_complex_der(heron, x0; h) - inv(2 * sqrt(x0))), hs
	)
	forwarddiff_err = abs(ForwardDiff.derivative(heron, x0) - inv(2 * sqrt(x0)))
...
@time ForwardDiff.derivative(Base.Fix2(heron, L_slider2), x0)

# ╔═╡ 8160588c-5ace-4c43-b2d4-043e8173944e
inv(2 * sqrt(x0))

# ╔═╡ f0b44125-c69e-44c6-80b2-4553e30c64d2
begin
	residuals(x) = A * x - b
...
# ╔═╡ 68e928c4-5acf-4096-a163-573b4c2fe749
begin
	sqnorm(r) = sum(abs2, r)
...
# ╔═╡ f188008f-f630-486c-9e2b-223bf4f71321
f = sqnorm ∘ residuals
...
J = jacobian_sparsity(f_sparse, x_test, sparsity_detector)
...
prep = DI.prepare_jacobian(f_sparse, backend, zero(x_test))
...
DI.jacobian(f_sparse, prep, backend, x_test)
